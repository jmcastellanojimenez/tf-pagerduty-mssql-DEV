...
resource "pagerduty_service" "legacy_jsf" {
...
resource "pagerduty_event_orchestration_service" "nonprod_services" {
  ...
  service = pagerduty_service.legacy_jsf.id
  set {
    id = "start"
...
resource "pagerduty_event_orchestration_service" "prod_services" {
  ...
  set {
   ...
   dynamic "rule" {
    ...
    rule {
         label = join("",["Set priority for JSF events to: ",local.def_prio])

         condition { 
            expression = "event.custom_details.NimProbeName matches 'JSF-Monitor'"
         }
         actions {
...
resource "pagerduty_service_dependency" "legacy_jsf_nonprod_app" {
  count = var.service_environment == "Production" ? 0 : 1 # Dont create this resource if its environment is Prod
  dependency {
    supporting_service {
      id   = pagerduty_service.legacy_jsf.id
      type = pagerduty_service.legacy_jsf.type
    }
    dependent_service {
...
resource "pagerduty_service_dependency" "legacy_jsf_prod_app" {
...
#  source_id         = pagerduty_service.k8s_cluster.id #This should be JSF-legacy-[prod|non-prod]
