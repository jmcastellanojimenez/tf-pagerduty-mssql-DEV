...
variable "service_business_prio" {
  type        = string
  default     = "p5"
  description = "Default priority to assign to events with no priority informed (Business Hours)."

  validation {
     condition = var.service_business_prio == "" || var.service_business_prio == "p1" || var.service_business_prio == "p2" || var.service_business_prio == "p3" || var.service_business_prio == "p4" || var.service_business_prio == "p5" 
     error_message = "Default priority must be from p1 to p5"
  }
}
...
variable "ads_context" {
  type = string
  default = ""
  description = "Link to the ADS context."
}
